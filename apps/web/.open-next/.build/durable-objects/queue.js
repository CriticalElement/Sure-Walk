...
globalThis.openNextVersion = "3.9.16";
...
var DOQueueHandler = class extends DurableObject {
    ...
    try {
      ...
      response = await this.service.fetch(`${protocol}://${host}${url}`, {
        ...
        headers: {
          ...
          "x-prerender-revalidate": "95fc2240f5312933252415059bbae3c1",
          "x-isr": "1",
        },
      ...
      if (
        response.status === 200 &&
        response.headers.get("x-nextjs-cache") !== "REVALIDATED"
      ) {
        ...
        throw new IgnorableError(
          `Something went wrong while revalidating ${host}${url}`,
        );
      } else if (response.status !== 200) {
        await this.addToFailedState(msg);
        throw new RecoverableError(
          `An unknown error occurred while revalidating ${host}${url}`,
        );
      }
  ...
  async alarm() {
    ...
    for (const event of allEventsToRetry) {
      debug(
        `Retrying revalidation for ${event.msg.MessageBody.host}${event.msg.MessageBody.url}`,
      );
  ...
  async addToFailedState(msg) {
    debug(
      `Adding ${msg.MessageBody.host}${msg.MessageBody.url} to the failed state`,
    );
    const existingFailedState = this.routeInFailedState.get(
      msg.MessageDeduplicationId,
    );
    ...
    if (existingFailedState) {
      ...
      const nextAlarmMs =
        Date.now() +
        Math.pow(2, existingFailedState.retryCount + 1) *
          this.revalidationRetryInterval;
      updatedFailedState = {
        ...existingFailedState,
  ...
  async addAlarm() {
    ...
    let nextAlarmToSetup = Math.min(
      ...Array.from(this.routeInFailedState.values()).map(
